...
from functions.Depth_to_color import Depth_to_irra_image
...
class DNet(nn.Module):
  def __init__(self, input_nc, output_nc, nf):
    ...
    self.Depth_to_irra_image = Depth_to_irra_image()
    self.avgpool_img_4 = nn.AvgPool2d(4)
    self.avgpool_img_2 = nn.AvgPool2d(2)
  def forward(self, x,x_albedo):
    out1 = self.layer1(x)
    ...
    #####   resolution 512  #######
    AB_512,normal_mag_512,mask_pix_num_512,mask_512 = self.Depth_to_irra_image(x,depth_512,refine_depth,x_albedo)
    x_norm_512 = (x+1.0)/2.0

    return refine_depth,depth_512+refine_depth,x_norm_512,AB_512
